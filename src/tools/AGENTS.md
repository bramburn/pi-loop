...
- `loop-tools.ts` — `LoopCreate` (with `priority` parameter), `LoopList`, `LoopPause`, `LoopResume`, `LoopUpdate`, `LoopInspect`. There is no `LoopDelete` tool — deletion is a user-driven action via the `/loop` command's View-loops menu, and internal cleanup paths (taskBacklog queue-drain auto-deletion) call `LoopStore.delete` directly. `LoopCreate` accepts a `priority: "defer" | "normal" | "urgent" | "critical"` enum (default `"normal"`). `LoopList` and `LoopUpdate` do not currently surface priority in their output — the priority is recorded in the loop entry and propagated to the runtime; surfacing it in `LoopList` is parked in the inline code review (rec #4 missing scope).
...
- **No `LoopDelete` tool.** Deletion is intentionally not in the LLM's tool surface. The user triggers deletion through the `/loop` command's View-loops menu, and internal cleanup paths (taskBacklog queue-drain auto-deletion, deletion tombstones) call `LoopStore.delete` directly. If a future change reintroduces a delete tool, do NOT expose it to the LLM — the LLM lacks the "user explicitly authorized this" signal that deletion requires.
