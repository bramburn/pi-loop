...
- `loop-tools.test.ts` — `LoopCreate`, `LoopList`, `LoopPause`, `LoopResume`, `LoopUpdate`, `LoopInspect`. There is no `LoopDelete` tool — deletion is user-driven via the `/loop` command. Tests include a regression guard asserting `toolMap.has("LoopDelete") === false`. Includes pause/resume happy path, idempotent re-resume, not-found, tombstone, and invalid-trigger rejection. `LoopCreate` priority test (`(priority as LoopPriority)` cast removed) is verified here.
